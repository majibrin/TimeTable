      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handleLogout}
            className="px-3 py-1.5 border border-red-200 text-red-600 text-xs font-bold rounded transition-colors">
            LOGOUT
          </button>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        {tab === 'MY COURSES' && (
          <div className="max-w-3xl">
                ...
                <div className="flex items-center py-1">
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input type="checkbox" checked={courseForm.hasPractical}
                      ...
                      className="w-3 h-3" />
                    <span className="text-xs text-slate-700 font-medium">This course requires lab/practical sessions</span>
                  </label>
                </div>
                  ...
                  <div className="border border-slate-200 rounded p-2 max-h-40 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-1">
                    {cohorts.map(c => (
                      <label key={c.id} className="flex items-center gap-1.5 cursor-pointer hover:bg-slate-50 p-1 rounded">
                        ...
                        <span className="text-[10px] text-slate-700">{c.level} ({c.student_count} students)</span>
                      </label>
                    ))}
                  </div>
                </div>
                <button type="submit" className="w-full py-2 bg-slate-900 text-white text-xs font-bold rounded hover:bg-slate-700 transition-colors">
                  SUBMIT FOR REVIEW
                </button>
              </form>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-lg shadow-sm">
              <div className="p-3 border-b border-slate-100 text-xs font-bold text-slate-700">MY SUBMITTED COURSES ({myCourses.length})</div>
              {myCourses.length === 0 ? (
                <div className="p-6 text-center text-xs text-slate-400">No courses submitted yet</div>
              ) : (
                  ...
                  {myCourses.map(c => (
                    <div key={c.id} className="p-3">
                      ...
                      {c.status === 'REJECTED' && c.officer_note && (
                        <div className="text-[10px] text-red-600 mt-1 font-medium">Rejection Reason: {c.officer_note}</div>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {tab === 'MY VENUES' && (
          <div className="max-w-xl">
            ...
            <div className="bg-white border border-slate-200 rounded-lg shadow-sm">
              <div className="p-3 border-b border-slate-100 text-xs font-bold text-slate-700">MY SUBMITTED VENUES ({myVenues.length})</div>
              {myVenues.length === 0 ? (
                <div className="p-6 text-center text-xs text-slate-400">No venues submitted yet</div>
              ) : (
                <div className="divide-y divide-slate-100 max-h-96 overflow-y-auto">
                  {myVenues.map(v => (
                    <div key={v.id} className="p-3">
                      ...
                      {v.status === 'REJECTED' && v.officer_note && (
                        <div className="text-[10px] text-red-600 mt-1 font-medium">Rejection Reason: {v.officer_note}</div>
                      )}
                    </div>
                  ))}
                </div>
