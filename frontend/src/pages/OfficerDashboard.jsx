...
const VIEW_LEVELS = ['ALL', '100L', '200L', '300L', '400L'];
const SCHEMES = [
...
export default function OfficerDashboard() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      setSchedules(slotsRes.data.map(slot => ({
        ...
        level: (slot.student_group_detail || slot.cohort_detail || '').match(/\b\d{3}L\b/)?.[0] || '',
  ...
  const handleExport = () => {
    ...
    exportTimetablePdf(exportSlice, {
      ...
      subtitle: `${selectedLevel === 'ALL' ? 'All Levels' : selectedLevel} — Generated by Timetable Officer Dashboard`,
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-2">
        <div className="max-w-7xl mx-auto flex flex-nowrap sm:flex-wrap items-center gap-1 sm:gap-2 overflow-x-auto">
          ...
          <button onClick={handleExport} disabled={generating}
            className="px-3 py-1.5 text-[10px] sm:text-xs font-bold border border-slate-300 hover:bg-slate-50 text-slate-700 rounded transition-colors whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed">
            EXPORT PDF
          </button>
          <button onClick={handlePublish} disabled={generating}
            className="px-3 py-1.5 text-[10px] sm:text-xs font-bold bg-green-600 hover:bg-green-700 text-white rounded transition-colors whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed">
            PUBLISH
          </button>
        </div>
      </div>
      ...
      {generating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 backdrop-blur-[1px]">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/10 p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-center gap-3 text-white">
              <div className="h-8 w-8 animate-spin rounded-full border-4 border-white/30 border-t-white" aria-label="loading" />
              <span className="text-sm font-bold uppercase tracking-[0.2em]">Generating timetable</span>
            </div>
            <div className="h-2 w-full overflow-hidden rounded-full bg-white/10">
              <div className="h-full w-full animate-[pulse_1.5s_ease-in-out_infinite] rounded-full bg-gradient-to-r from-blue-300 via-cyan-300 to-blue-500" />
            </div>
            <p className="mt-3 text-center text-[10px] uppercase tracking-[0.2em] text-slate-200">Please wait while the optimisation completes</p>
          </div>
        </div>
      )}
        ...
        {tab === 'TIMETABLE' && (
          loading
            ...
            : <>
              ...
              <div className="overflow-x-auto"><TimetableGrid schedules={selectedLevel === 'ALL' ? schedules : schedules.filter(slot => slot.level === selectedLevel)} /></div>
            </>
        )}
      ...
      {generating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70" role="status" aria-live="polite">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 text-center shadow-2xl">
            <div className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-4 border-slate-200 border-t-blue-600" />
            <p className="text-sm font-bold text-slate-800">Generating timetable...</p>
